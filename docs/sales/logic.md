...
### Work Order Fields (`order_headers`)
...
- `work_order_manual` is editable on `Sales\Order\Show` while status is `ORDER` or `DELIVERY` (permission `edit sales order`).
- Editing `work_order_manual` cascades to all `stock_adjustment_headers` linked via `order_header_id` so adjustments reflect the latest WO.
...
- **Work Order card** (visible when `work_order_auto` is set):
  ...
  - On `ORDER` / `DELIVERY` + `edit sales order`: shows `work_order_manual` input + Save button (`saveWorkOrderManual()`)
  - Save runs in a DB transaction, updates the SO and cascades the new `work_order_manual` to every `stock_adjustment_headers` row whose `order_header_id` equals this SO
