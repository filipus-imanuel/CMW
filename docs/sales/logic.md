...
| `work_order_manual` | string(100), nullable | Optional manual WO number. Edited via the Production module. |
| `production_date` | date, nullable | Optional production target date. Edited in the same window as `work_order_manual`. |
...
- `work_order_manual` and `production_date` are editable together on `Production\Edit` while SO status is `ORDER` or `DELIVERY` **and** `production_status` is not `finish` (permission `edit production order`).
- Saving on `Production\Edit::save()` cascades both `work_order_manual` and `production_date` to all `stock_adjustment_headers` linked via `order_header_id` so adjustments reflect the latest values.
...
- **Work Order card** (visible when `work_order_auto` is set):
  - Displays `work_order_auto`, `work_order_manual`, and `production_date` (all read-only)
  - WO Manual and Production Date are edited from `Production\Edit` (permission `edit production order`)
- **Refresh** button (always visible) reloads the order with its relations via `Show::refreshOrder()` so the page can be refreshed manually after edits made elsewhere (e.g. Production Edit)
