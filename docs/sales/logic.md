...
### Work Order / Production Fields (`order_headers`)

| Field | Type | Notes |
|-------|------|-------|
| `work_order_auto` | string(50), unique, nullable | Auto-generated on approval (`WO/YYMM/0001`). Immutable after issue. |
| `work_order_manual` | string(100), nullable | Optional manual WO number entered by sales staff after approval. |
| `production_date` | date, nullable | Optional production target date. Editable in the same window as `work_order_manual`. |
| `production_status` | string(20), default `ongoing` | `ongoing` or `finish`. Edited only via the Production module. |

- `work_order_auto` is generated by `CodeGeneratorHelper::generateWorkOrderCode()` during `Sales\Approval\Show::processApproval()`, alongside `code_order`.
- `work_order_manual` and `production_date` are editable together on `Sales\Order\Show` while status is `ORDER` or `DELIVERY` (permission `edit sales order`).
- Saving on `Sales\Order\Show::saveWorkOrderManual()` cascades both `work_order_manual` and `production_date` to all `stock_adjustment_headers` linked via `order_header_id` so adjustments reflect the latest values.
- `production_status` defaults to `ongoing` on order creation and is updated only through `Production\Edit` (permission `edit production order`).
- Composite index `(status, date)` exists on `order_headers` to keep the SO combobox lookup in Stock Adjustment fast.
...
| **Approve** | `approve sales order` | → `ORDER` + generate `code_order` + generate `work_order_auto` | `shp.sales.order.refresh.approval` + `shp.sales.order.refresh.ongoing` |
...
- Approve generates `work_order_auto` via `CodeGeneratorHelper::generateWorkOrderCode()` (format `WO/YYMM/0001`)
...
- **Work Order card** (visible when `work_order_auto` is set):
  - Displays `work_order_auto` (read-only)
  - On `ORDER` / `DELIVERY` + `edit sales order`: shows `work_order_manual` input and `production_date` date-picker + Save button (`saveWorkOrderManual()`)
  - Save runs in a DB transaction, updates the SO and cascades the new `work_order_manual` and `production_date` to every `stock_adjustment_headers` row whose `order_header_id` equals this SO
  - Dispatches `shp.sales.order.refresh`
